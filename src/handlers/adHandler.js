...
async function handleMessageForAds(bot, ctx) {
	...
	try {
		...
		console.log(
			`📩 Получено сообщение для анализа рекламы, ID: ${message.message_id}`
		)
		...
		const allowedThreadIds = process.env.AD_DETECTION_THREAD_IDS
			? process.env.AD_DETECTION_THREAD_IDS.split(',').map(id =>
					parseInt(id.trim(), 10)
			  )
			: [0, 1, 2] // По умолчанию разрешаем треды 0, 1, 2

		console.log(
			`💬 Проверка чата/треда: чат=${chatId}, тред=${threadId}, целевой чат=${
				config.MONO_PITER_CHAT_ID
			}, разрешенные треды=${allowedThreadIds.join(', ')}`
		)
		...
		if (!isTargetChat) {
			console.log(
				`❌ Сообщение из чата ${chatId}, не совпадает с целевым чатом ${config.MONO_PITER_CHAT_ID}`
			)
			return false
		}
		...
		if (!isAllowedThread) {
			console.log(
				`❌ Сообщение из треда ${threadId}, не входит в список разрешенных тредов: ${allowedThreadIds.join(
					', '
				)}`
			)
			return false
		...
		console.log(
			`📊 Результат анализа: вероятность рекламы ${analysisResult.adProbability}%, порог: ${config.MODULES.AD_DETECTION.MIN_PROBABILITY_THRESHOLD}%`
		)

		// Если вероятность рекламы выше порога, отправляем уведомление администраторам
		if (
			analysisResult.adProbability >=
			config.MODULES.AD_DETECTION.MIN_PROBABILITY_THRESHOLD
		) {
			console.log(
				`✅ Вероятность рекламы (${analysisResult.adProbability}%) превышает порог (${config.MODULES.AD_DETECTION.MIN_PROBABILITY_THRESHOLD}%)`
			)
...
`.trim()
				...
				console.log(
					`📤 Отправляем уведомление администраторам в чат ${config.MODULES.AD_DETECTION.REPORT_CHAT_ID}, тред ${config.MODULES.AD_DETECTION.REPORT_THREAD_ID} (AD_DETECTION_REPORT)`
				)
				try {
					await sendTelegramMessage(
						bot,
						config.MODULES.AD_DETECTION.REPORT_CHAT_ID,
						notification,
						{
							message_thread_id: config.MODULES.AD_DETECTION.REPORT_THREAD_ID,
							parse_mode: 'HTML',
					...
					console.error(
						'❌ Ошибка при отправке уведомления о рекламе:',
						sendError
					)
				}
				...
				console.log(
					'❌ Результат форматирования пустой, уведомление не отправлено'
				)
			}
		} else {
			console.log(
				`ℹ️ Вероятность рекламы (${analysisResult.adProbability}%) ниже порога (${config.MODULES.AD_DETECTION.MIN_PROBABILITY_THRESHOLD}%), пропускаем`
			)
		}
		...
		console.error(
			'❌ Ошибка при обработке сообщения на наличие рекламы:',
			error
		)
		return false
	}
