 ...
 async function handleNewChatMembers(bot, ctx) {
   ...
   try {
     ...
     try {
       ...
       await sendTelegramMessage(bot, new_chat_member.id, welcomeMessage, { parse_mode: 'HTML' });
       console.log(`✅ Приветственное сообщение отправлено пользователю ${new_chat_member.id}`);
     } catch (messageError) {
       // Вот тут был пропущен блок catch для внутреннего try
       console.error('❌ Не удалось отправить приветственное сообщение:', messageError);
       ...
       if (isUserAccessError(messageError)) {
         await sendTelegramMessage(
           bot,
           ADMIN_CHAT_ID,
           `Не удалось отправить приветственное сообщение пользователю ${new_chat_member.first_name} ${new_chat_member.last_name || ""}: ${formatUserAccessError(messageError, new_chat_member.id)}`,
           { message_thread_id: LAMP_THREAD_ID }
         );
       }
     }
     ...
     if (error.description?.includes('blocked') || error.description?.includes('initiate')) {
       await sendTelegramMessage(
         bot,
         ADMIN_CHAT_ID,
         `Произошла ошибка при обработке нового участника ${new_chat_member.first_name} ${new_chat_member.last_name || ""}: ${error.description}`,
         { message_thread_id: LAMP_THREAD_ID }
       );
     }
   }
 ...
 */
 ...
 Спасибо за понимание!
     ...
     try {
       ...
       if (messageSent === null) {
         console.log(`⚠️ Пользователь ${from.id} заблокировал бота - невозможно отправить инструкции`);
       ...
       console.error('❌ Ошибка при отправке сообщения пользователю:', messageError);
       
       // Если ошибка связана с доступом, добавляем информацию в сообщение для админов
       if (isUserAccessError(messageError)) {
         adminMessage += `\n\n⚠️ <b>ВНИМАНИЕ:</b> ${formatUserAccessError(messageError, from.id)}`;
     ...
     const askQuestionText = userStatus === "blocked_bot" 
     ...
     try {
       ...
       console.error('❌ Не удалось отправить уведомление админам о запросе:', adminMsgError);
     ...
     try {
       ...
       try {
         ...
         console.error('❌ Не удалось отправить уведомление об ошибке БД:', notifyError);
       }
     }
   } catch (error) {
