...
const { isUserAccessError, isRateLimitError, isCallbackQueryExpired, formatUserAccessError } = require('./errorHandler');
...
async function notifyAdminsAboutError(bot, error, context = {}) {
  try {
    const { userId, operation = 'неизвестная операция', additionalInfo = '' } = context;
    ...
    if (isUserAccessError(error)) {
      errorMessage += `\n📌 <b>Тип ошибки:</b> ${formatUserAccessError(error, userId || 'неизвестный')}\n`;
    ...
    ).catch(e => console.error('Failed to send error notification to admins:', e.message));
  } catch (notifyError) {
 ...
 */
async function sendTelegramMessage(bot, chatId, message, options = {}, errorOptions = {}) {
  try {
    ...
    const skipNotification = isUserAccessError(error) && errorOptions.skipUserAccessErrors === true;
 ...
 */
async function sendTelegramMedia(bot, chatId, content, options = {}, errorOptions = {}) {
  try {
