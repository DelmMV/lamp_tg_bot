...
async function analyzeMessageForAds(message, forceRefresh = false) {
  console.log(`🔍 Начало анализа сообщения ID: ${message.message_id} на наличие рекламы`)
  
  // Проверяем настройки модуля
  const { ENABLED, MIN_MESSAGE_LENGTH, CACHE_RESULTS } = config.MODULES.AD_DETECTION
  console.log(`⚙️ Настройки модуля: ENABLED=${ENABLED}, MIN_MESSAGE_LENGTH=${MIN_MESSAGE_LENGTH}, CACHE_RESULTS=${CACHE_RESULTS}`)
  ...
  console.log('- forward_from_chat:', message.forward_from_chat ? 'ЕСТЬ' : 'нет')
  console.log('- forward_from_message_id:', message.forward_from_message_id || 'нет')
  ...
  console.log('- forward_date:', message.forward_date ? new Date(message.forward_date * 1000).toISOString() : 'нет')
  ...
  const hasAnyForward = hasForwardOrigin || hasForwardFrom || hasForwardFromChat || !!message.forward_date
  
  console.log(`🔁 Результат проверки: сообщение ${hasAnyForward ? 'является пересланным' : 'не является пересланным'}`)
  ...
  if (isForwarded) {
    ...
    const source = message.forward_origin?.sender_user?.username || 
  ...
  console.log(`💬 Текст сообщения для анализа: "${messageText.substring(0, 100)}${messageText.length > 100 ? '...' : ''}" (длина: ${messageText.length})`)
  
  // Если сообщение переслано, уменьшаем минимальную длину для анализа
  const effectiveMinLength = isForwarded ? Math.max(10, MIN_MESSAGE_LENGTH / 2) : MIN_MESSAGE_LENGTH
  ...
  if (messageText.length < effectiveMinLength) {
    console.log(`❌ Сообщение слишком короткое для анализа (${messageText.length} < ${effectiveMinLength} символов)`)
    return {
      isLikelyAd: false,
      adProbability: 0,
      adTypes: [],
  ...
  console.log(`✅ Длина сообщения (${messageText.length}) достаточна для анализа (порог: ${effectiveMinLength})`)
  ...
  const messageInfo = {
    ...
    forward: {
      source: message.forward_origin?.sender_user?.username || 
              message.forward_origin?.chat?.title || 
              message.forward_from?.username || 
              message.forward_from?.first_name || 
              null,
      ...
      date: message.forward_date ? new Date(message.forward_date * 1000).toISOString() : null,
      isChannel: message.forward_origin?.chat?.type === 'channel',
      isGroup: message.forward_origin?.chat?.type === 'group',
      isUser: !!message.forward_from || message.forward_origin?.sender_user?.type === 'user'
    },
    // Информация о медиа и содержимом
    hasMedia: !!message.photo || !!message.video || !!message.document,
    mediaType: message.photo ? 'photo' : 
              message.video ? 'video' : 
              message.document ? 'document' : null,
    // Поиск специфических паттернов в тексте
    hasLinks: messageText.includes('http') || messageText.includes('t.me/') || messageText.includes('@'),
    hasPhoneNumbers: /\+?\d{10,15}/.test(messageText),
    hasEmails: /\S+@\S+\.\S+/.test(messageText),
    // Дополнительные паттерны для поиска рекламы
    hasPricePatterns: /\d+(\.\d+)? ?(\$|\u20ac|\u20bd|\u0440уб|руб|rub)/i.test(messageText),
    hasPromotionalWords: /(акция|скидка|распродажа|промокод|купон|бесплатно|sale|discount|promo|offer|buy|free)/i.test(messageText)
  }
...
- 0-20: точно не реклама
- 21-40: скорее не реклама
- 41-60: неопределенно
- 61-80: вероятно реклама
- 81-100: точно реклама

Учитывай контекст сообщения, наличие призывов к действию, рекламных фраз, ссылок, контактной информации и т.д.`
    ...
    console.log('📡 Отправляем запрос к OpenRouter API с моделью deepseek-chat-v3-0324:free')
    ...
    console.log('💬 Ответ от API:', responseText.substring(0, 200) + (responseText.length > 200 ? '...' : ''))
    ...
    if (CACHE_RESULTS) {
      console.log('💾 Сохраняем результат в кэш для сообщения ID:', message.message_id)
...
function formatAdAnalysisResult(analysisResult, message) {
  console.log('📝 Форматируем результат анализа рекламы для сообщения ID:', message.message_id)
  ...
  console.log(`📊 Данные для форматирования: вероятность=${adProbability}, типы=[${adTypes?.join(', ') || 'не указаны'}]`)
  ...
  if (adProbability < MIN_PROBABILITY_THRESHOLD) {
    console.log(`❌ Вероятность рекламы (${adProbability}%) ниже порога (${MIN_PROBABILITY_THRESHOLD}%), пропускаем`)
    return ''
  }
  
  console.log(`✅ Вероятность рекламы (${adProbability}%) превышает порог (${MIN_PROBABILITY_THRESHOLD}%), формируем результат`)
  ...
  if (message.forward_origin || message.forward_from) {
    console.log('📩 Сообщение является пересылкой, добавляем информацию об источнике')
    ...
    const sourceType = message.forward_origin?.chat?.type === 'channel' ? 'канала' : 
                      message.forward_origin?.chat?.type === 'group' ? 'группы' : 'пользователя'
...
📩 <b>Репост из ${sourceType}:</b> ${source}`
    ...
    if (message.forward_origin?.chat?.username && message.forward_origin?.message_id) {
      const originalLink = `https://t.me/${message.forward_origin.chat.username}/${message.forward_origin.message_id}`
      console.log(`🔗 Добавляем ссылку на оригинальное сообщение: ${originalLink}`)
      messageInfo += `
...
<b>Объяснение:</b> ${explanation}`
  ...
  console.log(`✅ Сформирован результат анализа для сообщения ID: ${message.message_id}`)
...
function cleanupCache() {
  ...
  if (expiredCount > 0) {
    console.log(`🧹 Очищено ${expiredCount} устаревших записей из кэша анализа рекламы`)
  }
}
